...
if [ $Reply = "master" ] || [ $Reply = "development" ] || [ $Reply = "stage" ]; then
    ...
    Url=$(jq '.sourceControl' devops.json)
        ...
        if [ -f ".gitlab-ci.yml" ]; then
            ...
            template='{"build-job":{"script":["aws codepipeline start-pipeline-execution --name %s --region ap-south-1"]}}'
            ...
            name="${repoFolder}-${Reply}-stage"
            template='{"build-job":{"when":"manual","variables":{"GIT_STRATEGY":"none"},"stage":"build","script":["aws codepipeline start-pipeline-execution --name %s --region ap-south-1"]}}'
            json_string=$(printf "$template" $name )
            echo $json_string > gitlab.json
